...
class CrawlingNaver():
    ...
    def get_info(self,url,area_big_idx:int):
        ...
        while True:
            ...
            for hotel in hotels:  
                ...
                hotel_url = "https://hotel.naver.com/hotels/item?hotelId=" + hotel_id
                etc = ""
                for num in range(1,4):
                    try:
    ...
    def run(self):
        ...
        for area_idx, area in enumerate(AddressConfig.AREA_NAVER):
            if area_idx >= 0:
                url = crawling_naver.init_url(area_idx)
                data_list = crawling_naver.get_info(url,area_idx)
                crawling_naver.save_file(data_list)
